...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			{
				pw.print("<h5> Login Successful from login servlet </h5>");// will appear on the clnt side
				 pw.flush();// no  IllegalStateExc on line no :  94 or 97
				...
				if (authenticatedUser.getRole().equals("CUSTOMER")) {
					...
					// 2. include the dyn contents from the 2nd page  in the SAME request
				...
				// WC : RETAINs pw's buffer ---suspends this exec -- invokes
				// TopicServlet's doPost --> after it rets
				// control comes back n optionally can generate more contents ---resp is sent to the clnt
				pw.print("<h5> Contents post inclusion....</h5>");
