...
public class TutorialDaoImpl implements ITutorialDao {
	...
	//	String jpql = "select t from Tutorial t join fetch t.topic tp where tp.topicName=:nm";
		...
		Transaction tx = session.beginTransaction();
		try {
			tuts = session.createQuery(jpql, Tutorial.class)
					.setParameter("nm", topicName).getResultList();
			tx.commit();
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
