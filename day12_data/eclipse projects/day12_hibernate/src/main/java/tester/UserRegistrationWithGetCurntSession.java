...
public class UserRegistrationWithGetCurntSession {
	...
	public static void main(String[] args) {
		try (SessionFactory sf = getSf(); Scanner sc = new Scanner(System.in)) {
			...
			User transientUser = new User(sc.next(), sc.next(), sc.next(), sc.next(), sc.next(),
					Role.valueOf(sc.next().toUpperCase()), sc.nextDouble(), LocalDate.parse(sc.next()));
			System.out.println(transientUser.getUserId());//null
			transientUser.setUserId(245);
			System.out.println(dao.addNewUserWithCurntSession(transientUser));
