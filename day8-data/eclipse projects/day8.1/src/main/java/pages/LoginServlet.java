...
public class LoginServlet extends HttpServlet {
	...
	//	System.out.println("ctx "+getServletContext());//NPE
	}
	
	public LoginServlet(String mesg) {
		System.out.println("in parmed ctor "+getServletConfig());//THIS WILL NOT get called
	}
	...
	public void init() throws ServletException {
		...
		// open connection
		try {
			// get servlet config from WC
			ServletConfig config = getServletConfig();
			System.out.println("config from init "+config);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			else // => login success
			{
				...
				// WC : CLEARS / DISCARDS pw's buffer ---send temp redirect resp , SC 302 |
				// location : topics
